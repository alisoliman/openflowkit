...
describe('HomePage integration flows', () => {
  ...
  it('exposes the full library and lets users narrow it to featured templates', async () => {
    await renderHomePage();
  ...
  it('removes the final remaining flow and shows the empty dashboard state when deleted', async () => {
    ...
    expect(document.activeElement).toBe(screen.getByTestId('home-create-new-header'));
  });
});
